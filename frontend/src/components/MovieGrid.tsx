...
export default function MovieGrid({ movies, onSelect, owned = {} }: Props) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 w-full">
      {movies.map((movie) => (
        <button
          key={movie.wikidata_id || `${movie.media_type}-${movie.tmdb_id}`}
        ...
        >
          <div className="aspect-[2/3] relative bg-zinc-800">
            ...
            {movie.wikidata_id && (
              <span title="Film není v TMDB — údaje z Wikidata / Wikipedie"
                className="absolute top-1.5 right-1.5 px-1.5 py-0.5 rounded text-[10px] font-bold bg-zinc-700 text-zinc-200">
                Wikidata
              </span>
            )}
